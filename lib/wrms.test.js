...
test('User lookup returns a user', done => {
  wrms.login()
    .then(function(res) {
      const params = {
        field: 'email',
        key: 'chrisburgess@catalyst.net.nz'
      };
      wrms.user.lookup(params)
        .then(function(res) {
          expect(typeof JSON.parse(res).response.users).toBe('object');
          expect(JSON.parse(res).response.users.length).toBe(1);
          done();
        });
    });
});
