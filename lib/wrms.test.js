...
const testData = {
  ...
  testWrSearchTerm: process.env.WRMS_TEST_WR_SEARCH_TERM || 'hapax legomenon'
}
...
test(`Work request search returns a WR [${testData.testWrSearchTerm}]`, done => {
  const params = {
    q: testData.testWrSearchTerm
  }
  wrms.wr.search(params)
    .then((res) => {
